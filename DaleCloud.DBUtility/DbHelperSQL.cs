...
namespace DaleCloud.DBUtility
{
    public static class DbHelperSQL
    {
        ...
        public static void BulkInsert<TModel>(List<TModel> modelList, string tableName)
        {
            var dtResult = ToSqlBulkCopyDataTable(modelList, tableName);

            var result = SqlBulkCopyHelper.BatchAdd(dtResult, tableName);
        ...
        public static DataTable ToSqlBulkCopyDataTable<TModel>(List<TModel> modelList, string tableName)
        {

            SqlParameter[] columnParameters = new SqlParameter[1];
            columnParameters[0] = new SqlParameter("@TableName", tableName);



            DataSet ds = DbHelperSQL.Query($"select syscolumns.Name,Colorder from syscolumns inner join sysobjects on syscolumns.id=sysobjects.id where sysobjects.xtype='U' and sysobjects.name='{tableName}' order by syscolumns.colid asc");

            IList<SysColumn> columns = DataListConvert.ConvertToList<SysColumn>(ds.Tables[0]);
